    ...
    for (x = 0; x != 258; ++x) {
      ...
      //yBound = MASK_Y / 2;            // 1
          ...
          if (x != 0 && y != 0 && x != 257 && y != 257) {
            R = *(source_bitmap +
                  bytes_per_pixel * (width * (y - 1) + (x - 1)) + 2);
            G = *(source_bitmap +
                  bytes_per_pixel * (width * (y - 1) + (x - 1)) + 1);
            B = *(source_bitmap +
                  bytes_per_pixel * (width * (y - 1) + (x - 1)) + 0);
          }
					else {
						R = 0;
						G = 0;
...
void Testbench::fetch_result() {
  unsigned int x, y; // for loop counter
  unsigned char red, green, blue;      // color of R, G, B
  ...
  for (y = 0; y != 258; ++y) {
    for (x = 0; x != 258; ++x) {
      ...
      if(x >= 2 && y >= 2){
        ...
        red = total.range(7, 0);
        green = total.range(15, 8);
        blue = total.range(23, 16);
        *(target_bitmap + bytes_per_pixel * (width * (y-2) + (x-2)) + 2) = red;
        *(target_bitmap + bytes_per_pixel * (width * (y-2) + (x-2)) + 1) = green;
        *(target_bitmap + bytes_per_pixel * (width * (y-2) + (x-2)) + 0) = blue;
      }
    }
  }
