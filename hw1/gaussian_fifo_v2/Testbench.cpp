...
void Testbench::do_gaussian() {
  ...
  for (y = -1; y != 257; ++y) {
	...
	B = 0;
      ...
      if(x >= 1 && y >= 1){
        if(i_red.num_available()==0) wait(i_red.data_written_event());
        red = i_red.read();

        if(i_green.num_available()==0) wait(i_green.data_written_event());
        green = i_green.read();

        if(i_blue.num_available()==0) wait(i_blue.data_written_event());
        blue = i_blue.read();
        //cout << "Now at " << sc_time_stamp() << endl; //print current sc_time
        

        *(target_bitmap + bytes_per_pixel * (width * (y-1) + (x-1)) + 2) = red;
        *(target_bitmap + bytes_per_pixel * (width * (y-1) + (x-1)) + 1) = green;
        *(target_bitmap + bytes_per_pixel * (width * (y-1) + (x-1)) + 0) = blue;
      }
