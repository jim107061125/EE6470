...
void LU::do_solve() {
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = i+1; j < n; ++j) {
      ...
      HLS_PIPELINE_LOOP(HARD_STALL, 1, "hard stall1");
      A[j][i] = A[j][i] << 6;
      A[j][i] /= A[i][i];
      //A[j][i] = A[j][i] >> 5;
    }
    ...
    for (int j = i+1; j < n; ++j) {
      for (int k = i+1; k < n; ++k) {
        ...
        HLS_PIPELINE_LOOP(HARD_STALL, 1, "hard stall2");
        A[j][k] -= (A[j][i] * A[i][k]) >> 6;
      }
    }
  }
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = i+1; j < n; ++j) {
      ...
      HLS_PIPELINE_LOOP(HARD_STALL, 1, "hard stall3");
      y[j] -= (A[j][i] * y[i]) >> 6;
    }
  }
  ...
  for (int i = n-1; i >= 0; i--) {
    ...
    //HLS_PIPELINE_LOOP(HARD_STALL, 1, "hard stall4");
    HLS_UNROLL_LOOP(ON, "SHIFT4");
    x[i] = x[i] << 6;
    x[i] /= A[i][i];
    //x[i] = x[i] >> 5;
    for (int j = i-1; j >= 0; j--) {
      ...
      HLS_PIPELINE_LOOP(HARD_STALL, 1, "hard stall5");
      //HLS_UNROLL_LOOP(ON, "SHIFT5");
      x[j] -= (A[j][i] * x[i]) >> 6;
    }
  } 
